...
int main(int argc, char *argv[])
  {
   ...
   DECLARE_TIMER(datainput);
   DECLARE_REPEAT_VAR(Loop);
   int Repeat = (1000);  
   ...
   if (1 != argc)
    ...
    START_TIMER(datainput);
    BEGIN_REPEAT_TIMING(Repeat,Loop); 
	...
	fclose(InputFile);
    END_REPEAT_TIMING;
    STOP_TIMER(datainput);

    DECLARE_TIMER(calculation);
    DECLARE_REPEAT_VAR(LoopControlVar);
    int NUM_ITERATIONS = (50000);
    START_TIMER(calculation);
    BEGIN_REPEAT_TIMING(NUM_ITERATIONS,LoopControlVar);
    ...
    PRINT_TIMER(datainput);
    PRINT_RTIMER(datainput,Repeat);
    PRINT_TIMER(calculation);
    PRINT_RTIMER(calculation,NUM_ITERATIONS);
    RESET_TIMER(datainput);
    RESET_TIMER(calculation);
    } /* if() */
  else
    {
   ...
   if (++DataSet->NextElement >= DataSet->Size)
     {
      ...
      int lcv;
      for (lcv = 0; lcv < tmp_old_size-2 ; lcv+=2)
        {
         NewData_X[lcv+0] = DataSet->Data_X[lcv+0];
         NewData_Y[lcv+0] = DataSet->Data_Y[lcv+0];
         NewData_X[lcv+1] = DataSet->Data_X[lcv+1];
         NewData_Y[lcv+1] = DataSet->Data_Y[lcv+1];
...
void CalculateCoefficients(LinearFit *DataSet, double *A, double *B)
  {
   ...
   for (lcv=0; lcv < DataSet->NextElement; lcv+=2)
    {
      S_XX += DataSet->Data_X[lcv+0] * DataSet->Data_X[lcv+0];
      S_XY += DataSet->Data_X[lcv+0] * DataSet->Data_Y[lcv+0];
      S_X  += DataSet->Data_X[lcv+0];
      S_Y  += DataSet->Data_Y[lcv+0];
